...
void insertion_sort(vector<Student>& s, bool compareStudent(const Student&, const Student&))
{
    ...
    for (int i = 1; i < s.size(); i++)
    {
        ...
        // The Student with the higher GPA should come first.  If they have
      // the same GPA, then the Student with the smaller id number should
      // come first.  Return true iff lhs should come first. 
      //1, 3, 2, 4 -> 3, 1, 2, 4 -> 3, 2, 1, 4
        while (j > 0  &&  compareStudent(s[j], s[j - 1]))
        {
            swap(s[j], s[j - 1]);
            j--;
        }
...
void sortUsingPtrs(vector<Student>& students, bool compareStudentPtr(const Student*, const Student*));
